...
class CF112{
	...
	public function getPDF($data, $templates, $sum_num){
		//токен пользователя
		...
		$data = array(
			...
			"whom" => iconv("windows-1251", "utf-8", $templates["to_surname"]),
			...
			"sum_num" => iconv("windows-1251", "utf-8", $sum_num),
			"message_part1" => "Оплата заказа № " . $_GET["order"],
		);

		$pdf = $api_pbrf->getBlank("pdf", "F112", $data, $login);
